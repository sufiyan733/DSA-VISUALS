...
//     timers.current.push(setTimeout(() => setMessage(null), 2600))
//   }

//   // ── BUG 6 FIX: accept newArr param so high is set to the CORRECT length ──
//   const resetSearch = (newArr) => {
//     const arr = newArr ?? array
//     clearTimers()
//     setLow(0)
//     setHigh(arr.length - 1)   // ← use passed-in array length, not stale state
//     setMid(-1)
//     setStatus('idle')
//     setHistory([])
//     stepsRef.current = 0
...
//     const arr = Array.from({ length: len }, () => Math.floor(Math.random() * 90) + 5)
//     arr.sort((a, b) => a - b)
//     setArray(arr)
//     setTarget('')
//     resetSearch(arr)           // ← pass new array so high = arr.length - 1
//     showMsg('New sorted array generated', 'info')
//   }

//   const stepSearch = () => {
//     if (status === 'found')    { showMsg('Already found! Reset or search again.', 'info'); return }
//     if (status === 'notfound') { showMsg('Not found. Reset or try another target.', 'info'); return }

//     // ── BUG 1 FIX: declare targetNum ONCE at the top, not twice ──
...
//       setMid(initialMid)
//       setHistory([{ low, high, mid: initialMid }])
//       stepsRef.current = 1
//       if (array[initialMid] === targetNum) {
//         setStatus('found')
//         showMsg(`Found ${targetNum} at index ${initialMid}!`, 'success')
//       } else {
//         showMsg(`arr[${initialMid}] = ${array[initialMid]} — ${array[initialMid] < targetNum ? 'go right ▶' : '◀ go left'}`, 'info')
//       }
//       return
//     }

//     // ── continue stepping ──
//     if (stepsRef.current >= MAX_STEPS) { showMsg('Too many steps – resetting', 'error'); resetSearch(); return }

//     // ── BUG 1 FIX: removed duplicate `const targetNum` that was here ──
//     if (low > high) { setStatus('notfound'); showMsg(`${targetNum} not found`, 'error'); return }
//     if (array[mid] === targetNum) { setStatus('found'); showMsg(`Found ${targetNum} at index ${mid}!`, 'success'); return }

//     let newLow  = low
//     let newHigh = high
//     if (array[mid] < targetNum) { newLow  = mid + 1 }
//     else                         { newHigh = mid - 1 }

//     if (newLow > newHigh) {
//       setLow(newLow); setHigh(newHigh); setMid(-1)
//       setStatus('notfound')
//       showMsg(`${targetNum} not found`, 'error')
//       return
//     }

//     const newMid = Math.floor((newLow + newHigh) / 2)
//     setLow(newLow); setHigh(newHigh); setMid(newMid)
//     setHistory(prev => [...prev, { low: newLow, high: newHigh, mid: newMid }])
//     stepsRef.current++

//     if (array[newMid] === targetNum) {
//       setStatus('found')
//       showMsg(`Found ${targetNum} at index ${newMid}!`, 'success')
//     } else {
//       showMsg(`arr[${newMid}] = ${array[newMid]} — ${array[newMid] < targetNum ? 'go right ▶' : '◀ go left'}`, 'info')
//     }
//   }
...
//     // ── BUG 4 FIX: run the algorithm first, THEN apply all state at once ──
//     let l = 0, h = array.length - 1, foundIdx = -1
//     const fullHistory = []
//     while (l <= h) {
//       const m = Math.floor((l + h) / 2)
//       fullHistory.push({ low: l, high: h, mid: m })
//       if      (array[m] === targetNum) { foundIdx = m; break }
//       else if (array[m] < targetNum)   { l = m + 1 }
//       else                              { h = m - 1 }
//     }

//     setHistory(fullHistory)
//     stepsRef.current = fullHistory.length

//     if (foundIdx !== -1) {
//       setLow(foundIdx); setHigh(foundIdx); setMid(foundIdx)
//       setStatus('found')
//       showMsg(`Found ${targetNum} at index ${foundIdx} in ${fullHistory.length} step${fullHistory.length > 1 ? 's' : ''}!`, 'success')
//     } else {
//       setLow(l); setHigh(h); setMid(-1)
//       setStatus('notfound')
//       showMsg(`${targetNum} not found — exhausted ${fullHistory.length} step${fullHistory.length !== 1 ? 's' : ''}`, 'error')
//     }
//   }

//   const msgStyle = {
//     success: { c: palette.accent,  b: `${palette.accent}40`  },
//     error:   { c: palette.danger,  b: `${palette.danger}40`  },
//     info:    { c: palette.textDim, b: `${palette.border}80`  },
//   }

//   // ── BUG 5 FIX: collect all pointer labels per index to avoid overlap ──
//   const pointers = {}
//   if (low  >= 0 && low  < array.length) pointers[low]  = [...(pointers[low]  || []), { lbl: 'LOW',  col: palette.primary }]
//   if (high >= 0 && high < array.length) pointers[high] = [...(pointers[high] || []), { lbl: 'HIGH', col: palette.warning }]
//   if (mid  >= 0 && mid  < array.length) pointers[mid]  = [...(pointers[mid]  || []), { lbl: 'MID',  col: palette.accent  }]
...
//         html,body{margin:0;padding:0;overflow:hidden;height:100%;background:${palette.darker};}
//         *,*::before,*::after{box-sizing:border-box;}

//         .shell {
//           height:calc(100vh - 52px);
//           background:radial-gradient(circle at 30% 40%, #1f1b3a, ${palette.darker});
...
//         .array-canvas{
//           flex:1;min-height:0;border-radius:24px;border:1px solid ${palette.border};
//           background:rgba(19,16,32,0.7);backdrop-filter:blur(8px);
//           box-shadow:0 20px 40px -15px rgba(0,0,0,0.7),inset 0 1px 2px rgba(255,255,255,0.08);
//           position:relative;overflow:hidden;
//           display:flex;align-items:center;justify-content:center;
//         }
//         .array-canvas::before{
//           content:'';position:absolute;inset:0;pointer-events:none;
//           background:linear-gradient(${palette.border}10 1px,transparent 1px),linear-gradient(90deg,${palette.border}10 1px,transparent 1px);
//           background-size:32px 32px;
//         }

//         .array-container{display:flex;gap:12px;flex-wrap:wrap;justify-content:center;padding:28px 20px 20px;}

//         /* BUG 5 FIX: extra top padding so stacked pointer labels don't clip */
//         .cell-wrap{position:relative;padding-top:0;}
//         .pointer-stack{
//           position:absolute;bottom:calc(100% + 4px);left:50%;transform:translateX(-50%);
//           display:flex;flex-direction:column;align-items:center;gap:3px;pointer-events:none;
//         }
//         .pointer-label{
//           font-family:'Syne';font-weight:700;font-size:8px;
//           background:${palette.dark};padding:2px 7px;border-radius:10px;
//           border:1px solid currentColor;white-space:nowrap;
//         }

//         .array-cell{
//           width:66px;height:66px;border-radius:16px;border:1px solid ${palette.border};
//           background:rgba(8,6,16,0.7);backdrop-filter:blur(4px);
//           display:flex;flex-direction:column;align-items:center;justify-content:center;
//           box-shadow:0 8px 12px -6px rgba(0,0,0,0.5);
//         }
//         .array-index{font-family:'JetBrains Mono';font-size:9px;color:${palette.textDim};margin-bottom:4px;}
//         .array-value{font-family:'JetBrains Mono';font-weight:700;font-size:17px;color:${palette.text};}

//         .panel{flex:1;min-height:0;display:flex;flex-direction:column;gap:8px;}
//         .card{background:rgba(19,16,32,0.8);backdrop-filter:blur(8px);border:1px solid ${palette.border};border-radius:20px;padding:12px 16px;flex-shrink:0;box-shadow:0 10px 20px -10px rgba(0,0,0,0.5);}
//         .card.fill{flex:1;min-height:0;display:flex;flex-direction:column;}
//         .clabel{font-family:'Syne';font-weight:700;font-size:9px;color:${palette.textDim};letter-spacing:2.5px;text-transform:uppercase;margin-bottom:8px;flex-shrink:0;}

//         .btn-act{flex:1;border:none;border-radius:12px;padding:9px 6px;font-family:'Syne';font-size:11px;font-weight:700;cursor:pointer;transition:all 0.2s;background:rgba(8,6,16,0.7);backdrop-filter:blur(4px);border:1px solid ${palette.border};color:${palette.text};}
//         .btn-act:hover:not(:disabled){transform:translateY(-2px);border-color:currentColor;}
//         .btn-act:disabled{opacity:0.22;cursor:not-allowed;}
//         .b-step  {color:${palette.accent};}
//         .b-full  {color:${palette.primary};}
//         .b-reset {color:${palette.warning};}
//         .b-random{color:${palette.info};}

//         .target-inp{background:rgba(8,6,16,0.7);backdrop-filter:blur(4px);border:1px solid ${palette.border};border-radius:14px;padding:10px 14px;color:${palette.text};font-family:'JetBrains Mono';font-size:13px;outline:none;transition:border-color 0.2s;width:100%;}
//         .target-inp:focus{border-color:${palette.primary}80;box-shadow:0 0 0 3px ${palette.primary}20;}
//         .target-inp::-webkit-inner-spin-button{-webkit-appearance:none;}
//         .target-inp[type=number]{-moz-appearance:textfield;}

//         /* step history */
//         .hist-row{display:flex;gap:6px;align-items:center;padding:4px 0;border-bottom:1px solid ${palette.border}20;}
...
//             <h1 style={{ fontFamily:'Syne', fontWeight:800, fontSize:'1.8rem', color:palette.text, textShadow:'0 2px 5px rgba(0,0,0,0.5)', margin:0 }}>
//               Binary Search
//             </h1>
//             <span style={{ fontFamily:'JetBrains Mono', fontSize:'10px', color:palette.info, background:`${palette.info}20`, border:`1px solid ${palette.info}40`, borderRadius:'20px', padding:'2px 12px' }}>
//               O(log n)
//             </span>
//             <span style={{ fontFamily:'JetBrains Mono', fontSize:'10px', color:palette.textDim, marginLeft:'auto' }}>
...
//                 const isLow  = index === low  && status !== 'idle'
//                 const isHigh = index === high && status !== 'idle'
//                 const isFound= status === 'found' && mid === index

//                 // ── BUG 2 FIX: eliminated — history used in right panel below ──
//                 // ── BUG 5 FIX: use pre-computed pointers map ──
//                 const cellPtrs = pointers[index] ?? []

//                 let borderColor = palette.border
//                 let bgColor     = 'rgba(8,6,16,0.7)'
//                 let glow        = 'none'
//                 let valColor    = palette.text

//                 if (isFound) {
//                   borderColor = palette.accent; bgColor = `${palette.accent}22`
//                   glow        = `0 0 24px ${palette.accent}70`; valColor = palette.accent
//                 } else if (isMid) {
//                   borderColor = palette.accent; bgColor = `${palette.accent}18`
//                   glow        = `0 0 16px ${palette.accent}50`
//                 } else if (isLow && isHigh) {
//                   borderColor = palette.secondary; bgColor = `${palette.secondary}15`
//                 } else if (isLow) {
//                   borderColor = palette.primary;  bgColor = `${palette.primary}15`
//                 } else if (isHigh) {
//                   borderColor = palette.warning;  bgColor = `${palette.warning}15`
//                 } else if (
//                   status !== 'idle' &&
//                   index >= low && index <= high && mid >= 0
//                 ) {
//                   borderColor = `${palette.border}`; bgColor = 'rgba(14,10,28,0.7)'
//                 } else if (status !== 'idle') {
//                   // eliminated range — dim
//                   borderColor = `${palette.border}50`; bgColor = 'rgba(5,4,14,0.5)'; valColor = '#334155'
//                 }

//                 return (
//                   <motion.div key={index} className="cell-wrap"
//                     initial={{ opacity:0, scale:0.5 }}
//                     animate={{ opacity:1, scale:1 }}
//                     transition={{ delay: index * 0.03, type:'spring', stiffness:300 }}
//                   >
//                     {/* ── BUG 5 FIX: render all pointer labels stacked ── */}
//                     {cellPtrs.length > 0 && (
//                       <div className="pointer-stack">
//                         {cellPtrs.map(({ lbl, col }) => (
//                           <motion.span key={lbl} className="pointer-label"
...
//                     <motion.div className="array-cell"
//                       animate={{ borderColor, background:bgColor, boxShadow:glow }}
//                       transition={{ duration:0.25 }}
//                     >
//                       <span className="array-index">[{index}]</span>
//                       <span className="array-value" style={{ color:valColor }}>{value}</span>
//                     </motion.div>
//                   </motion.div>
//                 )
//               })}
...
//           {/* message */}
//           <div style={{ height:'32px', marginTop:'8px', marginBottom:'8px', flexShrink:0 }}>
//             <AnimatePresence mode="wait">
//               {message && (
//                 <motion.div key={message.text}
//                   initial={{opacity:0,y:3}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-3}}
//                   style={{ background:'rgba(19,16,32,0.8)', backdropFilter:'blur(8px)', border:`1px solid ${msgStyle[message.type].b}`, borderRadius:'30px', padding:'5px 16px', fontFamily:'JetBrains Mono', fontSize:'11px', color:msgStyle[message.type].c, height:'100%', display:'flex', alignItems:'center', boxShadow:'0 4px 12px rgba(0,0,0,0.3)' }}>
//                   {message.text}
//                 </motion.div>
//               )}
//             </AnimatePresence>
...
//             <input className="target-inp" placeholder="target number" type="number" value={target}
//               onChange={e => setTarget(e.target.value)}
//               onKeyDown={e => e.key === 'Enter' && stepSearch()} />
//           </div>
//           <div style={{ display:'flex', gap:'8px', flexShrink:0 }}>
//             <button className="btn-act b-step"   onClick={stepSearch}        disabled={status==='found'||status==='notfound'}>⏭ Step</button>
//             <button className="btn-act b-full"   onClick={searchFull}        disabled={!target.trim()}>⚡ Full Search</button>
//             <button className="btn-act b-reset"  onClick={() => resetSearch()}>↺ Reset</button>
//             <button className="btn-act b-random" onClick={generateRandomArray}>⟳ Random</button>
//           </div>
//         </div>
...
//             <h2 style={{ fontFamily:'Syne', fontWeight:800, fontSize:'1.8rem', color:palette.text, textShadow:'0 2px 5px rgba(0,0,0,0.5)', margin:0 }}>Info</h2>
//             <p style={{ fontFamily:'JetBrains Mono', fontSize:'10px', color:palette.textDim, marginTop:'4px' }}>efficient search on sorted arrays</p>
//           </div>
...
//                     <span style={{ fontFamily:'JetBrains Mono', fontSize:'9px', color:palette.primary, background:`${palette.primary}15`, border:`1px solid ${palette.primary}30`, borderRadius:'50%', width:'18px', height:'18px', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>{i+1}</span>
//                     <span style={{ fontFamily:'JetBrains Mono', fontSize:'10px', color:palette.textDim }}>{s}</span>
//                   </div>
//                 ))}
//               </div>
//             </div>
...
//                 {[
//                   { lbl:'Time (avg)',   val:'O(log n)', col:palette.accent  },
//                   { lbl:'Time (worst)', val:'O(log n)', col:palette.accent  },
//                   { lbl:'Space',        val:'O(1)',      col:palette.info    },
//                   { lbl:'Precondition', val:'Sorted',    col:palette.warning },
//                 ].map(({lbl,val,col})=>(
//                   <div key={lbl} style={{ background:'rgba(8,6,16,0.6)', border:'1px solid '+palette.border, borderRadius:'12px', padding:'8px 10px' }}>
//                     <p style={{ fontFamily:'JetBrains Mono', fontSize:'8px', color:palette.textDim, marginBottom:'3px', margin:0 }}>{lbl}</p>
//                     <p style={{ fontFamily:'JetBrains Mono', fontWeight:700, fontSize:'14px', color:col, margin:0 }}>{val}</p>
//                   </div>
//                 ))}
//               </div>
//             </div>

//             {/* ── BUG 2 FIX: history panel — actually renders the history state ── */}
//             <div className="card" style={{ flexShrink:0 }}>
//               <p className="clabel">Step History  <span style={{ color:palette.primary }}>({history.length})</span></p>
//               {history.length === 0 ? (
//                 <p style={{ fontFamily:'JetBrains Mono', fontSize:'10px', color:`${palette.border}`, margin:0 }}>
//                   No steps yet — press Step or Full Search
//                 </p>
//               ) : (
//                 <div style={{ display:'flex', flexDirection:'column', maxHeight:'90px', overflowY:'auto' }}>
//                   {history.map((h, i) => (
//                     <div key={i} className="hist-row">
//                       <span style={{ fontFamily:'JetBrains Mono', fontSize:'8px', color:palette.textDim, minWidth:'20px' }}>#{i+1}</span>
//                       <span style={{ fontFamily:'JetBrains Mono', fontSize:'9px', color:palette.primary }}>low={h.low}</span>
//                       <span style={{ fontFamily:'JetBrains Mono', fontSize:'9px', color:palette.warning }}>high={h.high}</span>
//                       <span style={{ fontFamily:'JetBrains Mono', fontSize:'9px', color:palette.accent  }}>mid={h.mid}  [{array[h.mid]}]</span>
//                     </div>
//                   ))}
//                 </div>
//               )}
...
//               <p className="clabel">Applications</p>
//               <div style={{ flex:1, display:'flex', flexDirection:'column', gap:'5px', overflow:'hidden' }}>
//                 {[
//                   { icon:'🔍', text:'Dictionary / word lookup'       },
//                   { icon:'📚', text:'Library catalog search'         },
//                   { icon:'🧮', text:'Numerical root finding'         },
//                   { icon:'📊', text:'Database indexing (B-trees)'    },
//                   { icon:'🎮', text:'Game spatial partitioning'      },
//                 ].map(({ icon, text }) => (
//                   <div key={text} style={{ background:'rgba(8,6,16,0.6)', border:`1px solid ${palette.border}`, borderRadius:'10px', padding:'7px 12px', display:'flex', alignItems:'center', gap:'8px', flex:1 }}>
//                     <span style={{ fontSize:'13px' }}>{icon}</span>
//                     <span style={{ fontFamily:'JetBrains Mono', fontSize:'10px', color:palette.textDim }}>{text}</span>
//                   </div>
//                 ))}
//               </div>
//             </div>
