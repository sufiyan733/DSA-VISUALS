...
// ═══════════════════════════════════════════════════════════════════════════════
// VOICE ENGINE — Fixed 1.25x, premium voice priority
// ═══════════════════════════════════════════════════════════════════════════════
const RATE = 1.25;
...
// STICKY NAV
// ═══════════════════════════════════════════════════════════════════════════════
function StickyNav({ active, speaking }) {
  ...
  return (
    <nav style={{
      ...
      background:"rgba(6,8,18,0.9)", backdropFilter:"blur(28px) saturate(180%)",
      ...
      {speaking && (
        <div style={{
          ...
          <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:9, color:"#4ade80", fontWeight:700 }}>1.25×</span>
        </div>
      )}
    </nav>
  );
...
function Hero({ onStart, onVoice }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ position:"absolute",top:"45%",left:"45%",width:500,height:500,borderRadius:"50%",
        transform:"translate(-50%,-50%)",
        background:"radial-gradient(circle,rgba(129,140,248,0.04) 0%,transparent 65%)",
        filter:"blur(48px)",pointerEvents:"none" }}/>

      {/* Tree */}
      <div style={{ width:"100%",maxWidth:480,marginBottom:44 }}>
      ...
      <div style={{ maxWidth:660, position:"relative" }}>
        ...
        <p style={{
          margin:"0 auto 38px",fontFamily:"'DM Sans',sans-serif",
          fontSize:"clamp(15px,2.2vw,19px)",color:"#64748b",lineHeight:1.68,maxWidth:540,
        }}>
          Every major tree concept — animated, explained, and narrated at <strong style={{ color:"#818cf8" }}>1.25× premium voice</strong>.
          Go from zero to confident in one sitting.
        </p>

        <div style={{ display:"flex",gap:14,justifyContent:"center",flexWrap:"wrap" }}>
          <button onClick={onStart} style={{
            padding:"16px 38px",borderRadius:16,cursor:"pointer",
          ...
          <button onClick={onVoice} style={{
            padding:"16px 28px",borderRadius:16,cursor:"pointer",
        ...
        <div style={{ display:"flex",gap:30,justifyContent:"center",marginTop:52,flexWrap:"wrap" }}>
          {[["9","Sections"],["7+","Animations"],["6","Quiz Qs"],["1.25×","Voice Speed"]].map(([n,l]) => (
...
// SECTION WRAPPER
// ═══════════════════════════════════════════════════════════════════════════════
function Sect({ id, icon, title, color, visual, cards, voice, speaking, onVoice }) {
  ...
  return (
    <section id={id} ref={ref} style={{
      ...
      <div style={{ display:"flex",alignItems:"center",gap:14,marginBottom:30 }}>
        ...
        <h2 style={{
          flex:1,margin:0,
          fontFamily:"'Syne',sans-serif",fontSize:"clamp(21px,3.8vw,32px)",
          fontWeight:800,color:"#f8fafc",letterSpacing:"-0.022em",lineHeight:1.15,
        }}>{title}</h2>
        <button onClick={() => onVoice(id, voice)} style={{
          display:"flex",alignItems:"center",gap:7,
          padding:"8px 17px",borderRadius:28,cursor:"pointer",flexShrink:0,
          ...
          animation:isSp?"sPulse 2s ease-in-out infinite":"none",
        }}>
          <span style={{ fontSize:13 }}>{isSp?"⏸":"🔊"}</span>
          {isSp?"STOP":"LISTEN"}
        </button>
      </div>
      ...
      <div className="sg" style={{
        display:"grid",gridTemplateColumns:"minmax(0,1.12fr) minmax(0,0.88fr)",gap:20,
      }}>
        <div style={{
          ...
          {cards.map((c,i) => (
            <div key={i} style={{
              ...
              {c.lbl && <div style={{
                ...
                color,letterSpacing:"0.12em",marginBottom:6,opacity:0.88,
              }}>{c.lbl}</div>}
              <div style={{ fontFamily:"'DM Sans',sans-serif",fontSize:13.2,color:"#94a3b8",lineHeight:1.7 }}>
                {c.body}
              </div>
            </div>
          ))}
...
function VisIntro() {
  ...
  const NS = [
    {id:0,x:200,y:52, v:"A",par:null},
    {id:1,x:110,y:132,v:"B",par:0},
    {id:2,x:290,y:132,v:"C",par:0},
    {id:3,x:62, y:208,v:"D",par:1},
    {id:4,x:158,y:208,v:"E",par:1},
    {id:5,x:242,y:208,v:"F",par:2},
    {id:6,x:338,y:208,v:"G",par:2},
    {id:7,x:40, y:280,v:"H",par:3},
    {id:8,x:84, y:280,v:"I",par:3},
  ];
  const COLS = ["#818cf8","#38bdf8","#38bdf8","#4ade80","#4ade80","#4ade80","#4ade80","#f472b6","#f472b6"];
  return (
    <svg viewBox="0 0 400 312" width="100%" style={{ maxHeight:290 }}>
      ...
      {NS.map((n,i)=>{
        ...
        return (
          <g key={n.id} style={{animation:`vPp 0.5s cubic-bezier(0.22,1,0.36,1) ${i*0.09}s both`}}>
            {ip&&<circle cx={n.x} cy={n.y} r={r+15}
              fill="none" stroke={c} strokeWidth="1" strokeOpacity="0.22"
              style={{animation:"vRip 0.8s ease-out forwards"}}/>}
            <circle cx={n.x} cy={n.y} r={r}
              fill={`${c}1a`} stroke={c} strokeWidth={ip?2.5:1.5}
              filter={ip?"url(#vgI)":"none"} style={{transition:"all 0.3s"}}/>
            <text x={n.x} y={n.y+1} textAnchor="middle" dominantBaseline="middle"
              fill={c} fontSize={i===0?13:11} fontFamily="'JetBrains Mono',monospace" fontWeight="700">{n.v}</text>
          </g>
        );
      })}
...
function VisAnatomy() {
  ...
  return (
    <div>
      <svg viewBox="-12 10 424 238" width="100%" style={{ maxHeight:248 }}>
        ...
        {ND.slice(3).map((n,i)=>(
          <text key={i} x={n.x} y={n.y+1} textAnchor="middle" dominantBaseline="middle"
            fill="#4ade80" fontSize="10" fontFamily="'JetBrains Mono',monospace" fontWeight="700">L</text>
        ))}
        {LBL.map(l=>(
          <g key={l.id} onMouseEnter={()=>setHov(l.id)} onMouseLeave={()=>setHov(null)} style={{cursor:"default"}}>
            <line x1={l.nx} y1={l.ny} x2={l.lx} y2={l.ly}
              stroke={hov===l.id?l.c:"rgba(255,255,255,0.1)"} strokeWidth={hov===l.id?1.5:1}
              strokeDasharray="4,3" style={{transition:"stroke 0.2s"}}/>
            <text x={l.lx} y={l.ly} fill={hov===l.id?l.c:"#3d4860"}
              fontSize="9" fontFamily="'JetBrains Mono',monospace" fontWeight="700"
              style={{transition:"fill 0.22s"}}>{l.t}</text>
          </g>
        ))}
      </svg>
      <div style={{
        minHeight:38,padding:"8px 14px",borderRadius:10,marginTop:4,
...
function VisTypes() {
  ...
  return (
    <div>
      ...
      <div style={{ padding:14,borderRadius:14,background:`${T.c}09`,border:`1px solid ${T.c}22` }}>
        <svg viewBox="0 0 280 200" width="100%" style={{ maxHeight:190 }}>
          ...
          {T.nodes.map(([x,y,r],i)=>(
            <g key={`${tab}-${i}`} style={{animation:`vPp 0.4s cubic-bezier(0.22,1,0.36,1) ${i*0.07}s both`}}>
              ...
              {i===0&&<text x={x} y={y+1} textAnchor="middle" dominantBaseline="middle"
                fill={T.c} fontSize="11" fontFamily="'JetBrains Mono',monospace" fontWeight="700">R</text>}
            </g>
          ))}
        ...
        <p style={{ textAlign:"center",marginTop:8,fontFamily:"'DM Sans',sans-serif",fontSize:13,color:"#94a3b8",margin:"8px 0 0" }}>
...
function VisBST() {
  ...
  const ND = [
    {id:0,x:200,y:55, v:50,par:null},
    {id:1,x:120,y:130,v:30,par:0},
    {id:2,x:280,y:130,v:70,par:0},
    {id:3,x:75, y:210,v:20,par:1},
    {id:4,x:165,y:210,v:40,par:1},
    {id:5,x:235,y:210,v:60,par:2},
  ...
  return (
    <div>
      ...
      <svg viewBox="0 0 400 262" width="100%" style={{maxHeight:252}}>
        ...
        {vis.map(n=>{
          ...
          return (
            <g key={n.id} style={{animation:"bstP 0.5s cubic-bezier(0.22,1,0.36,1) both"}}>
              ...
              <circle cx={n.x} cy={n.y} r={20} fill={`${col}20`} stroke={col}
                strokeWidth={(iC||iF)?2.5:1.5}
                filter={(iC||iF)?"url(#bGlw)":"none"} style={{transition:"all 0.3s"}}/>
              <text x={n.x} y={n.y+1} textAnchor="middle" dominantBaseline="middle"
                fill={col} fontSize="12" fontFamily="'JetBrains Mono',monospace" fontWeight="700">{n.v}</text>
            </g>
          );
        })}
        ...
        {mode==="search"&&found!==null&&(
          <text x={200} y={252} textAnchor="middle" fill="#4ade80" fontSize="10" fontFamily="'JetBrains Mono',monospace">
            ✓ Found {sTarget} in {SPATHS[sTarget]?.length} comparisons
          </text>
        )}
  ...
  return (
    <div>
      <div style={{display:"flex",gap:5,justifyContent:"center",marginBottom:10,flexWrap:"wrap",alignItems:"center"}}>
        ...
        <button onClick={()=>setAuto(true)} style={{padding:"4px 10px",borderRadius:20,cursor:"pointer",background:auto?"rgba(255,255,255,0.06)":"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.1)",fontFamily:"'JetBrains Mono',monospace",fontSize:9,color:"#475569"}}>AUTO</button>
      </div>
      <svg viewBox="0 0 400 252" width="100%" style={{maxHeight:242}}>
        <defs>
        ...
        {ph===1&&<path d="M 265 94 Q 210 140 157 114" fill="none" stroke="#fbbf24"
          strokeWidth="2.5" markerEnd="url(#avlArr)" strokeDasharray="6,3"
          style={{animation:"avlDash 1s linear infinite"}}/>}
        {S.nodes.map((n,i)=>(
...
function VisHeap() {
  ...
  const MAX=[
    {id:0,x:200,y:50, v:90,par:null},
    {id:1,x:120,y:130,v:70,par:0},
    {id:2,x:280,y:130,v:80,par:0},
    {id:3,x:75, y:210,v:50,par:1},
    {id:4,x:165,y:210,v:60,par:1},
    {id:5,x:235,y:210,v:65,par:2},
    {id:6,x:325,y:210,v:75,par:2},
  ];
    ...
    <div>
      ...
      <svg viewBox="0 0 400 252" width="100%" style={{maxHeight:242}}>
        {ND.filter(n=>n.par!==null).map(n=>{
          ...
          return <line key={n.id} x1={pr.x} y1={pr.y} x2={n.x} y2={n.y}
            stroke={isH?col:`${col}38`} strokeWidth={isH?2.5:1.5} style={{transition:"stroke 0.3s"}}/>;
        })}
        {ND.map((n,i)=>{
          const iR=n.par===null;
          const isH=hl.includes(n.id);
          return (
            <g key={`${type}-${n.id}`} style={{animation:`hpIn 0.4s cubic-bezier(0.22,1,0.36,1) ${i*0.06}s both`}}>
              ...
              <circle cx={n.x} cy={n.y} r={iR?25:20}
                fill={isH?`${col}35`:iR?`${col}28`:`${col}0e`}
                stroke={isH?col:iR?col:`${col}72`}
                strokeWidth={iR?2.5:1.5} style={{transition:"all 0.35s"}}/>
              <text x={n.x} y={n.y+1} textAnchor="middle" dominantBaseline="middle"
                fill={iR?col:"#cbd5e1"} fontSize={iR?14:12} fontFamily="'JetBrains Mono',monospace" fontWeight="700">{n.v}</text>
            </g>
          );
        })}
        <text x={200} y={244} textAnchor="middle" fill="#2d3748" fontSize="9" fontFamily="'JetBrains Mono',monospace">
          Array: [{ND.map(n=>n.v).join(", ")}]
        </text>
...
function VisTrie() {
  ...
  const TN=[
    {id:"root",x:200,y:35, lbl:"∅",par:null},
    {id:"c",   x:130,y:105,lbl:"c",par:"root"},
    {id:"d",   x:265,y:105,lbl:"d",par:"root"},
    {id:"ca",  x:90, y:175,lbl:"a",par:"c"},
    {id:"co",  x:170,y:175,lbl:"o",par:"c"},
    {id:"do",  x:240,y:175,lbl:"o",par:"d"},
    {id:"dn",  x:310,y:175,lbl:"n",par:"d"},
    {id:"cat", x:55, y:245,lbl:"t✓",par:"ca"},
    {id:"car", x:100,y:245,lbl:"r✓",par:"ca"},
    {id:"cak", x:145,y:245,lbl:"k",par:"ca"},
    {id:"cap", x:190,y:245,lbl:"p✓",par:"ca"},
    {id:"dog", x:240,y:245,lbl:"g✓",par:"do"},
  ...
  return (
    <div>
      ...
      <svg viewBox="0 0 380 272" width="100%" style={{maxHeight:268}}>
        {TN.filter(n=>n.par).map(n=>{
          const pr=NM[n.par];
          const hl=hlSet.has(n.id)&&hlSet.has(n.par);
          return <line key={n.id} x1={pr.x} y1={pr.y} x2={n.x} y2={n.y}
            stroke={hl?"#a855f7":"rgba(255,255,255,0.1)"} strokeWidth={hl?2.5:1} style={{transition:"all 0.3s"}}/>;
        })}
        {TN.map(n=>{
          const hl=hlSet.has(n.id);
          const isE=n.lbl.includes("✓");
          const isR=n.id==="root";
          ...
          return (
            <g key={n.id}>
              ...
              <circle cx={n.x} cy={n.y} r={r}
                fill={hl?"rgba(168,85,247,0.3)":isE?"rgba(74,222,128,0.12)":"rgba(255,255,255,0.04)"}
                stroke={hl?"#a855f7":isE?"#4ade80":"rgba(255,255,255,0.14)"}
                strokeWidth={hl?2.5:1.5} style={{transition:"all 0.3s"}}/>
              <text x={n.x} y={n.y+1} textAnchor="middle" dominantBaseline="middle"
                fill={hl?"#a855f7":isE?"#4ade80":"#5a6b84"} fontSize="10" fontFamily="'JetBrains Mono',monospace" fontWeight="700">{n.lbl}</text>
            </g>
          );
        })}
      </svg>
...
function VisTraversal() {
  ...
  const ND=[
    {id:0,x:200,y:55, v:4,par:null},
    {id:1,x:120,y:130,v:2,par:0},
    {id:2,x:280,y:130,v:6,par:0},
    {id:3,x:75, y:210,v:1,par:1},
    {id:4,x:165,y:210,v:3,par:1},
    {id:5,x:235,y:210,v:5,par:2},
    {id:6,x:325,y:210,v:7,par:2},
  ];
  const ORDS={
  ...
  return (
    <div>
      ...
      <svg viewBox="0 0 400 248" width="100%" style={{maxHeight:238}}>
        ...
        {ND.map(n=>{
          ...
          const isC=CUR.sq[step-1]===n.id;
          const ord=CUR.sq.indexOf(n.id);
          return (
            <g key={n.id}>
              ...
              <circle cx={n.x} cy={n.y} r={20}
                fill={isC?`${CUR.c}35`:isV?`${CUR.c}12`:"rgba(255,255,255,0.05)"}
                stroke={isC?CUR.c:isV?`${CUR.c}72`:"rgba(255,255,255,0.14)"}
                strokeWidth={isC?2.5:1.5} style={{transition:"all 0.35s"}}/>
              {isV&&<text x={n.x+26} y={n.y-14} fill={CUR.c} fontSize="10" fontFamily="'JetBrains Mono',monospace" fontWeight="700">{ord+1}</text>}
              <text x={n.x} y={n.y+1} textAnchor="middle" dominantBaseline="middle"
                fill={isC?CUR.c:isV?"#94a3b8":"#e2e8f0"} fontSize="13" fontFamily="'JetBrains Mono',monospace" fontWeight="700">{n.v}</text>
            </g>
          );
        })}
      ...
      <p style={{textAlign:"center",fontFamily:"'JetBrains Mono',monospace",fontSize:9,color:CUR.c,marginBottom:8,margin:"0 0 8px"}}>{CUR.d}</p>
      ...
      <table style={{width:"100%",borderCollapse:"collapse"}}>
        <thead>
          <tr>{["Structure","Search","Insert","Delete","Notes"].map(h=>(
            <th key={h} style={{padding:"10px 14px",textAlign:"left",fontFamily:"'JetBrains Mono',monospace",fontSize:9,letterSpacing:"0.1em",color:"#2d3748",borderBottom:"1px solid rgba(255,255,255,0.06)",fontWeight:700}}>{h}</th>
          ))}</tr>
        </thead>
        <tbody>
          {rows.map((r,i)=>(
              ...
              <td style={{padding:"10px 14px"}}>
                <div style={{display:"flex",alignItems:"center",gap:8}}>
                  <span style={{width:8,height:8,borderRadius:"50%",background:r.c,flexShrink:0,
                    boxShadow:hov===i?`0 0 8px ${r.c}`:"none",transition:"box-shadow 0.2s"}}/>
                  <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:11,fontWeight:700,color:"#e2e8f0"}}>{r.nm}</span>
                </div>
              </td>
              {[r.s,r.i,r.d].map((v,j)=>(
                <td key={j} style={{padding:"10px 14px",fontFamily:"'JetBrains Mono',monospace",fontSize:11,
                  fontWeight:v.includes("log")||v==="O(m)"?700:400,
                  color:v.includes("log")||v==="O(m)"?"#4ade80":"#ef4444"}}>{v}</td>
              ))}
...
function Quiz({ onDone }) {
  const QS = [
    {q:"Which BST traversal ALWAYS produces sorted output?",
     opts:["Pre-order","In-order","Post-order","Level-order"],ans:1,
     exp:"In-order (L→Root→R) visits BST nodes in ascending order — this is a fundamental and frequently tested property."},
    {q:"What is the worst-case height of a BST built by inserting 1, 2, 3, 4, 5, 6, 7 in order?",
     opts:["3","6","4","log₂(7) ≈ 3"],ans:1,
     exp:"Inserting sorted data creates a right-skewed chain. Height = n−1 = 6. This is the degenerate BST case."},
    {q:"AVL balance factor is defined as:",
     opts:["Value of node","height(left) − height(right)","Number of children","Depth from root"],ans:1,
     exp:"BF = height(left subtree) − height(right subtree). AVL guarantees |BF| ≤ 1 at every single node."},
    {q:"In a heap stored as an array, what is the index of the LEFT child of node at index i?",
     opts:["i + 1","2i","2i + 1","(i − 1) / 2"],ans:2,
     exp:"Left child = 2i+1, Right = 2i+2, Parent = ⌊(i−1)/2⌋. No pointers needed — this is what makes heaps memory-efficient."},
    {q:"Trie search for a word of length m has complexity:",
     opts:["O(n)","O(m · log n)","O(m)","O(log n)"],ans:2,
     exp:"O(m) — completely independent of n (total words stored). Add a million more words: search speed is unchanged."},
    {q:"Which structure gives O(1) access to the maximum element?",
     opts:["AVL Tree","BST","Max Heap","Trie"],ans:2,
     exp:"Max Heap always keeps the maximum at the root. Peek = O(1). Extract = O(log n). Perfect for priority queues."},
  ];
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:18}}>
      {QS.map((q,qi)=>{
        ...
        return (
          <div key={qi} style={{padding:"18px 20px",borderRadius:16,background:"rgba(255,255,255,0.02)",border:`1px solid ${bc}`,transition:"border-color 0.3s"}}>
            <div style={{display:"flex",gap:10,marginBottom:13,alignItems:"flex-start"}}>
              <span style={{
                width:25,height:25,borderRadius:9,flexShrink:0,marginTop:1,
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              {q.opts.map((opt,oi)=>{
                const isSel=ans[qi]===oi;
                const isCorr=q.ans===oi;
                ...
                return (
                  <button key={oi} onClick={()=>!isR&&setAns(a=>({...a,[qi]:oi}))} style={{
                    padding:"10px 14px",borderRadius:10,cursor:isR?"default":"pointer",
                    background:bg,border:`1px solid ${brd}`,
                    fontFamily:"'DM Sans',sans-serif",fontSize:13,color:col,
                    textAlign:"left",transition:"all 0.22s",display:"flex",alignItems:"center",gap:9,
                  }}>
                    <span style={{
            ...
            {isR&&(
              <div style={{marginTop:10,padding:"10px 14px",borderRadius:10,background:ans[qi]===q.ans?"rgba(74,222,128,0.07)":"rgba(239,68,68,0.07)",border:`1px solid ${ans[qi]===q.ans?"rgba(74,222,128,0.2)":"rgba(239,68,68,0.2)"}`,fontFamily:"'DM Sans',sans-serif",fontSize:12.5,color:"#94a3b8",lineHeight:1.58,animation:"fUp 0.3s ease"}}>
...
export default function TreePage() {
  ...
  const handleVoice = useCallback((id, text) => {
    if (speaking === id) { voiceStop(); setSpeaking(null); }
    else { setSpeaking(id); voiceSpeak(text, () => setSpeaking(null)); }
  }, [speaking]);
    ...
    <div style={{ background:"#060810", color:"#f8fafc", minHeight:"100vh", overflowX:"hidden" }}>
      <style>{`
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        @media(max-width:760px){.sg{grid-template-columns:1fr !important}}
        @media(max-width:480px){.sg{gap:12px !important}}
      `}</style>

      <ProgressBar/>
      <StickyNav active={active} speaking={speaking}/>
      ...
      <main style={{ maxWidth:1000, margin:"0 auto", padding:"0 24px 100px" }}>
        ...
        <section id="complexity" style={{ marginBottom:92 }}>
          <div style={{ display:"flex",alignItems:"center",gap:14,marginBottom:24 }}>
            ...
            <div>
              <h2 style={{ fontFamily:"'Syne',sans-serif",fontSize:"clamp(21px,3.8vw,32px)",fontWeight:800,color:"#f8fafc" }}>Complexity Cheat Sheet</h2>
        ...
        {/* ── Quiz Section ──────────────────────────────────────────── */}
        <section id="quiz" style={{ marginBottom:92 }}>
          <div style={{ display:"flex",alignItems:"center",gap:14,marginBottom:8 }}>
            ...
            <div style={{ flex:1 }}>
              <h2 style={{ fontFamily:"'Syne',sans-serif",fontSize:"clamp(21px,3.8vw,32px)",fontWeight:800,color:"#f8fafc" }}>Test Your Knowledge</h2>
            ...
            <button onClick={()=>handleVoice("quiz",NARR.quiz)} style={{
              display:"flex",alignItems:"center",gap:7,
              padding:"8px 16px",borderRadius:28,cursor:"pointer",flexShrink:0,
              ...
              animation:speaking==="quiz"?"sPulse 2s ease-in-out infinite":"none",
            }}>
              <span style={{ fontSize:13 }}>{speaking==="quiz"?"⏸":"🔊"}</span>
          ...
          {qScore !== null && (
            <div style={{
              marginTop:34,padding:"38px 28px",borderRadius:26,textAlign:"center",
              ...
              <p style={{ fontFamily:"'DM Sans',sans-serif",fontSize:15,color:"#94a3b8",margin:"10px 0 26px",lineHeight:1.55 }}>
                {qScore>=5
        ...
        {/* ── Footer CTA ───────────────────────────────────────────── */}
        <div style={{
          textAlign:"center",padding:"58px 28px",borderRadius:28,
          ...
          <h3 style={{ fontFamily:"'Syne',sans-serif",fontSize:"clamp(18px,3.2vw,28px)",fontWeight:800,color:"#f8fafc",marginBottom:14 }}>
            You have completed the Tree guide!
          </h3>
          <p style={{ fontFamily:"'DM Sans',sans-serif",fontSize:14,color:"#64748b",maxWidth:460,margin:"0 auto 30px",lineHeight:1.72 }}>
          ...
          <p style={{ fontFamily:"'JetBrains Mono',monospace",fontSize:9,color:"#1a2030",letterSpacing:"0.1em" }}>
            ALL CODE BUTTONS REDIRECT → /stack
          </p>
        </div>
