...
class SoundEngine {
  ...
  playTyping() {
    ...
    const now = Date.now();
    if (now - this.lastTypeTime < 40) return; // throttle
    ...
    try {
      ...
      const freqs = [880, 1046, 1174, 1318, 1568];
...
// ─────────────────────────────────────────────────────────────────────────────
function TreeFlowchart({ data }) {
  if (!data || !data.nodes || data.nodes.length === 0) return null;
  const { nodes, edges, title } = data;
  
  // Layout: auto position if not provided
  const W = 560, H = Math.max(240, nodes.length * 48);
  const levelMap = {};
  nodes.forEach(n => {
    const lvl = n.level || 0;
  ...
  levels.forEach(lvl => {
    ...
    group.forEach((n, i) => {
      const x = (i + 1) * W / (group.length + 1);
      const y = 40 + lvl * 72;
      positionedNodes[n.id] = { ...n, x, y };
    });
  });

  const typeColors = {
    root: { fill: "rgba(109,120,250,0.25)", stroke: ACCENT2, text: TEXT_PRI },
    step: { fill: "rgba(56,189,248,0.15)", stroke: ACCENT3, text: TEXT_SEC },
    leaf: { fill: "rgba(52,211,153,0.12)", stroke: "#34d399", text: "#34d399" },
    warn: { fill: "rgba(244,114,182,0.15)", stroke: "#f472b6", text: "#f472b6" },
    info: { fill: "rgba(165,180,252,0.12)", stroke: ACCENT4, text: ACCENT4 },
  };

  const svgH = Math.max(260, (levels.length) * 72 + 80);
  ...
  return (
    <div style={{
      ...
      {title && (
        <div style={{
          padding: "8px 14px",
          borderBottom: "1px solid rgba(109,120,250,0.15)",
          fontFamily: MONO, fontSize: "9px", color: ACCENT4,
          letterSpacing: "0.08em",
          background: "rgba(109,120,250,0.06)",
          display: "flex", alignItems: "center", gap: "6px",
        }}>
          <span style={{ color: ACCENT3 }}>⊕</span>
          {title.toUpperCase()}
          <span style={{
            marginLeft: "auto", fontSize: "8px", color: TEXT_DIM,
      ...
      <div style={{ overflowX: "auto", padding: "8px" }}>
        <svg width="100%" viewBox={`0 0 ${W} ${svgH}`} style={{ display: "block", minWidth: "280px" }}>
          <defs>
            <marker id="arrowhead" markerWidth="8" markerHeight="6" refX="6" refY="3" orient="auto">
              <polygon points="0 0, 8 3, 0 6" fill={ACCENT2} opacity="0.7"/>
            </marker>
            <filter id="glow-node">
          ...
          {/* Edges */}
          {(edges || []).map((e, i) => {
            const from = positionedNodes[e.from];
            const to = positionedNodes[e.to];
            ...
            return (
              <g key={i}>
                <path
                  d={`M${from.x},${from.y + 18} C${from.x},${my} ${to.x},${my} ${to.x},${to.y - 18}`}
                  fill="none" stroke={ACCENT2} strokeWidth="1.5" strokeOpacity="0.5"
                  markerEnd="url(#arrowhead)"
                  strokeDasharray={e.dashed ? "4,3" : "none"}
                />
                {e.label && (
                  <text x={mx} y={my - 4} textAnchor="middle"
                    fill={ACCENT4} fontSize="8" fontFamily={MONO} opacity="0.7">{e.label}</text>
                )}
              </g>
            );
          })}
          ...
          {/* Nodes */}
          {Object.values(positionedNodes).map(n => {
            const style = typeColors[n.type || "step"] || typeColors.step;
            const w = Math.min(140, Math.max(80, n.label.length * 7 + 20));
            const h = 36;
            const shape = n.type === "root" ? "diamond" : n.shape || "rect";
              ...
              <g key={n.id} filter="url(#glow-node)">
                {shape === "diamond" ? (
                  ...
                  <rect x={n.x - w/2} y={n.y - h/2} width={w} height={h}
                    rx="16" fill={style.fill} stroke={style.stroke} strokeWidth="1.5"/>
                ) : (
                  <rect x={n.x - w/2} y={n.y - h/2} width={w} height={h}
                    rx="8" fill={style.fill} stroke={style.stroke} strokeWidth="1.5"/>
                )}
                <text x={n.x} y={n.y + 1} textAnchor="middle" dominantBaseline="middle"
                  fill={style.text} fontSize="9.5" fontFamily={SANS} fontWeight="600">
                  {n.label.length > 16 ? n.label.slice(0, 14) + "…" : n.label}
                </text>
                {n.sub && (
                  <text x={n.x} y={n.y + 14} textAnchor="middle"
                    fill={style.text} fontSize="7" fontFamily={MONO} opacity="0.55">
                    {n.sub.length > 20 ? n.sub.slice(0, 18) + "…" : n.sub}
                  </text>
                )}
              </g>
            );
...
function HeaderBtn({ icon, tooltip, onClick, variant="default", disabled=false, active=false }) {
  ...
  const styles = {
    ...
    sound:   { idle:TEXT_DIM, hover:"#34d399", bg:"transparent", hBg:"rgba(52,211,153,0.1)", b:"transparent", hB:"rgba(52,211,153,0.3)" },
  };
...
// ─────────────────────────────────────────────────────────────────────────────
// MARKDOWN RENDERER — strips ### headers, shows plain text
// ─────────────────────────────────────────────────────────────────────────────
function RichText({ content, flowchartData }) {
  // strip leading ### ## # from headings → just show text
  const cleaned = content.replace(/^(#{1,6})\s+/gm, "");
  const parts = cleaned.split(/(```[\s\S]*?```|\[FLOWCHART:[^\]]*\])/g);
  return (
    <div style={{ margin:0, lineHeight:"1.72", wordBreak:"break-word" }}>
      {parts.map((part, i) => {
        ...
        if (part.startsWith("[FLOWCHART:") && flowchartData) {
          return <TreeFlowchart key={i} data={flowchartData}/>;
        }
      ...
      {flowchartData && !content.includes("[FLOWCHART:") && (
        <TreeFlowchart data={flowchartData}/>
      )}
    </div>
  );
}
...
// ─────────────────────────────────────────────────────────────────────────────
function StreamingMessage({ fullText, onDone, streamKey, flowchartData }) {
  ...
  return <RichText content={displayed} flowchartData={null}/>;
}
...
function VoicePickerModal({ currentVoice, onSelect, onClose, voiceSpeed, onSpeedChange }) {
  ...
  const testVoice = (v, idx) => {
    ...
    utt.voice = v; utt.rate = voiceSpeed;
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ marginBottom:"18px", padding:"12px 14px", borderRadius:"10px",
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:"6px" }}>
          ...
          {availableVoices.map((v, i) => {
            ...
            const qualityColors = { NEURAL:"#34d399", ENHANCED:ACCENT3, ONLINE:ACCENT4, GOOGLE:"#f472b6" };
            return (
              <div key={i} style={{
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"18px" }}>
          <div>
            <div style={{ fontFamily:DISPLAY, fontSize:"13px", color:TEXT_PRI, fontWeight:700, letterSpacing:"0.04em" }}>Bookmarks</div>
            <div style={{ fontFamily:MONO, fontSize:"9px", color:TEXT_DIM, marginTop:"3px" }}>{bookmarks.length} saved</div>
          </div>
          <button onClick={onClose} style={{
            width:"28px", height:"28px", borderRadius:"8px",
            background:"rgba(255,255,255,0.04)", border:`1px solid ${BORDER}`,
    ...
    <div style={{
      ...
      <div style={{ width:"100%", maxWidth:"540px" }}>
        <div style={{
          ...
          />
          {query && (
            <button onClick={()=> setQuery("")} style={{ background:"none", border:"none", color:TEXT_DIM, cursor:"pointer", outline:"none", display:"flex", alignItems:"center" }}>
              <IconX size={12}/>
            </button>
          )}
          <button onClick={onClose} style={{
...
function ShortcutsModal({ onClose }) {
  const shortcuts = [
    ...
    { keys:["Ctrl","Shift","S"],   desc:"Toggle sound effects" },
  ];
  return (
    <div style={{
...
// ─────────────────────────────────────────────────────────────────────────────
// REACTION BAR — new UX extra
// ─────────────────────────────────────────────────────────────────────────────
function ReactionBar({ msgIdx, reactions, onReact }) {
  const emojis = ["👍","🔥","💡","⭐","🤔"];
  return (
    <div style={{ display:"flex", gap:"3px", marginTop:"4px" }}>
      {emojis.map(e => {
        const count = reactions?.[msgIdx]?.[e] || 0;
        return (
          <button key={e} onClick={()=> onReact(msgIdx, e)} style={{
            display:"flex", alignItems:"center", gap:"2px",
            background: count > 0 ? "rgba(109,120,250,0.15)" : "rgba(255,255,255,0.03)",
            border:`1px solid ${count > 0 ? "rgba(109,120,250,0.3)" : "rgba(255,255,255,0.05)"}`,
            borderRadius:"20px", padding:"2px 6px",
            fontSize:"10px", cursor:"pointer", outline:"none",
            transition:"all 0.15s",
          }}>
            {e}
            {count > 0 && <span style={{ fontFamily:MONO, fontSize:"7px", color:ACCENT4 }}>{count}</span>}
          </button>
        );
      })}
    </div>
...
// MESSAGE BUBBLE
// ─────────────────────────────────────────────────────────────────────────────
function MessageBubble({ msg, idx, speakingIdx, onSpeak, onStopSpeak, voiceSpeed, onSpeedChange, isBookmarked, onBookmark, isStreaming, isMobile, reactions, onReact }) {
  ...
  if (!isUser && isMobile) {
    return (
      <div className="bot-msg-in" style={{ width:"100%", animationDelay:`${Math.min(idx*0.04, 0.3)}s` }}>
        <div style={{ display:"flex", alignItems:"center", gap:"8px", marginBottom:"8px" }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:"6px" }}>
            <span style={{ fontFamily:MONO, fontSize:"9px", fontWeight:700, color:ACCENT2, letterSpacing:"0.12em" }}>VISUOSLAYER AI</span>
            <div style={{ width:"4px", height:"4px", borderRadius:"50%", background:"#34d399", boxShadow:"0 0 5px #34d399" }}/>
            {isStreaming && !streamDone && (
              <span style={{ fontFamily:MONO, fontSize:"8px", color:ACCENT3, animation:"bot-antenna-pulse 1s infinite" }}>GENERATING...</span>
            )}
          </div>
        </div>
        <div style={{
          ...
          background:"linear-gradient(145deg,rgba(255,255,255,0.035) 0%,rgba(30,27,75,0.2) 100%)",
          border:"1px solid rgba(255,255,255,0.06)",
          boxShadow:"0 2px 12px rgba(0,0,0,0.4),inset 0 1px 0 rgba(255,255,255,0.03)",
          position:"relative", overflow:"hidden",
        }}>
          <div style={{ position:"absolute", top:0, left:"12px", right:"12px", height:"1px",
            background:`linear-gradient(90deg,transparent,${ACCENT}50,transparent)` }}/>
          <div style={{ fontFamily:SANS, fontSize:"13.5px", lineHeight:"1.72", color:TEXT_SEC, fontWeight:400 }}>
            {isStreaming && !streamDone ?
              <StreamingMessage fullText={msg.content} onDone={()=> setStreamDone(true)} streamKey={msg.streamKey}/> :
              <RichText content={msg.content} flowchartData={msg.flowchart}/>
            }
          </div>
          {!isStreaming || streamDone ? <ReactionBar msgIdx={idx} reactions={reactions} onReact={onReact}/> : null}
          <div ref={speedRef} style={{
            display:"flex", alignItems:"center", justifyContent:"space-between",
            marginTop:"10px", paddingTop:"8px",
            borderTop:"1px solid rgba(255,255,255,0.04)", position:"relative",
          }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:"4px" }}>
              ...
              {showSpeeds && (
                <div style={{
                  ...
                  borderRadius:"10px", overflow:"hidden", boxShadow:"0 12px 32px rgba(0,0,0,0.6)",
              ...
              <button onClick={()=> isSpeaking ? onStopSpeak() : onSpeak(msg.content, idx)} style={{
                ...
                {isSpeaking ? <IconVolumeOff size={13}/> : <IconVolume size={13}/>}
              </button>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="bot-msg-in" style={{
      ...
      <div style={{
        maxWidth: isUser ? "min(78%, 620px)" : "min(84%, 720px)",
        width:"fit-content",
        ...
        {!isUser && (!isStreaming || streamDone) && (
          <ReactionBar msgIdx={idx} reactions={reactions} onReact={onReact}/>
        )}
...
function LanguageSelector({ currentLang, onSelect, onClose }) {
  const langs = [
    ...
    { name:"Kotlin",     icon:"🟣", color:"#7c3aed" },
    { name:"Swift",      icon:"🍎", color:"#f43f5e" },
  ];
  return (
    <div style={{
      ...
      <div style={{
        ...
        padding:"28px 24px", width:"min(460px,calc(100vw - 28px))",
        boxShadow:"0 32px 80px rgba(0,0,0,0.7),inset 0 1px 0 rgba(255,255,255,0.07)",
          ...
          {langs.map(lang => {
            ...
            return (
              <button key={lang.name} onClick={()=> onSelect(lang.name)} style={{
                ...
                cursor:"pointer", fontFamily:SANS, fontSize:"12px", fontWeight:600,
                transition:"all 0.18s cubic-bezier(0.22,1,0.36,1)", outline:"none", textAlign:"left",
                boxShadow: sel ? `0 4px 16px ${lang.color}20` : "none",
              }}
                onMouseEnter={e=>{ if(!sel){ e.currentTarget.style.background=`${lang.color}12`; e.currentTarget.style.borderColor=`${lang.color}35`; e.currentTarget.style.color=TEXT_PRI; }}}
                onMouseLeave={e=>{ if(!sel){ e.currentTarget.style.background="rgba(255,255,255,0.02)"; e.currentTarget.style.borderColor="rgba(255,255,255,0.06)"; e.currentTarget.style.color=TEXT_SEC; }}}
              >
                <span style={{ fontSize:"15px", flexShrink:0 }}>{lang.icon}</span>
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ fontWeight:600, fontSize:"12px" }}>{lang.name}</div>
                </div>
                {sel && <div style={{ width:"7px", height:"7px", borderRadius:"50%", background:lang.color, boxShadow:`0 0 8px ${lang.color}`, flexShrink:0 }}/>}
              </button>
            );
          })}
        </div>
...
export default function ChatBot() {
  ...
  // ── FIX 1: Lock body scroll when chatbot is open, z-index above everything ──
  useEffect(() => {
    if (open) {
  ...
  useEffect(() => {
    ...
    setParticles(Array.from({ length:12 }, (_,i) => ({
      id:i, top:`${5+(i*8.1)%90}%`, left:`${5+((i*37)%90)}%`,
  ...
  // ── FIX 7: prevent wheel events from bubbling out of chatbot scroll area ──
  useEffect(() => {
  ...
  const sendMessage = useCallback(async (text) => {
    ...
    try {
      ...
      // Determine if topic warrants a flowchart
      const flowchartKeywords = ["explain","how","works","steps","process","algorithm","sort","tree","graph","dp","dynamic","traverse","search","path","structure","bfs","dfs","avl","heap","trie","hash"];
      const wantsFlowchart = flowchartKeywords.some(kw => userText.toLowerCase().includes(kw));
...
6. Concept → intuition → complexity → code structure.
7. NEVER use ### or ## or # headings — write section labels as plain bold text like **Core Concept** instead.
${wantsFlowchart ? `8. After your explanation, output a JSON flowchart inside a <FLOWCHART> XML tag. Format exactly:
<FLOWCHART>
{"title":"Topic Name","nodes":[{"id":"1","label":"NodeLabel","level":0,"type":"root"},{"id":"2","label":"Step","level":1,"type":"step"}],"edges":[{"from":"1","to":"2"}]}
</FLOWCHART>
Use types: root (diamond), step (rectangle), leaf (rounded), warn (warning), info (info). Level 0 = top.` : ""}`;
      ...
      // Extract flowchart JSON if present
      let flowchartData = null;
      const fcMatch = reply.match(/<FLOWCHART>([\s\S]*?)<\/FLOWCHART>/);
      if (fcMatch) {
        try {
          flowchartData = JSON.parse(fcMatch[1].trim());
        } catch {}
        reply = reply.replace(/<FLOWCHART>[\s\S]*?<\/FLOWCHART>/g, "").trim();
      }

      setMessages(prev => {
        const next = [...prev, { role:"assistant", content:reply, time:getTime(), streamKey, flowchart: flowchartData }];
  ...
  const chatPaddingH = isMobile ? "12px" : "clamp(16px, 5vw, 80px)";
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing:border-box; }
        ...
        @keyframes bot-shimmer { 0%{background-position:-200% 0} 100%{background-position:200% 0} }
        @keyframes bot-panel-in { from{opacity:0;transform:translateY(18px)} to{opacity:1;transform:translateY(0)} }
        @keyframes bot-panel-out { from{opacity:1;transform:translateY(0)} to{opacity:0;transform:translateY(18px)} }
        ...
        @keyframes bot-badge-pop {
          0%{transform:scale(0) rotate(-20deg)} 70%{transform:scale(1.25) rotate(5deg)} 100%{transform:scale(1) rotate(0)}
        }
        @keyframes bot-glow-ring {
          0%,100%{opacity:0.5;transform:scale(1)} 50%{opacity:1;transform:scale(1.1)}
        }
        @keyframes bot-scan-line {
          0%{top:-1px;opacity:0} 10%{opacity:0.7} 90%{opacity:0.7} 100%{top:100%;opacity:0}
        }
        ...
        @keyframes bot-grid-move { 0%{transform:translate(0,0)} 100%{transform:translate(24px,24px)} }
        ...
        @keyframes bot-send-ready {
          0%,100%{box-shadow:0 4px 14px rgba(109,120,250,0.4)} 50%{box-shadow:0 4px 22px rgba(56,189,248,0.5)}
        }
        ...
        @keyframes bot-empty-float {
          0%,100%{transform:translateY(0) rotate(0deg)} 33%{transform:translateY(-5px) rotate(1deg)} 66%{transform:translateY(-2px) rotate(-0.8deg)}
        }
        @keyframes bot-ring-expand {
          0%{transform:scale(0.8);opacity:0.8} 100%{transform:scale(2.2);opacity:0}
        }
        @keyframes bot-modal-in {
          from{opacity:0;transform:scale(0.96) translateY(8px)} to{opacity:1;transform:scale(1) translateY(0)}
        }
        @keyframes bot-tip-in { from{opacity:0;transform:translateY(-3px)} to{opacity:1;transform:translateY(0)} }
        @keyframes bot-mic-pulse {
          0%,100%{box-shadow:0 0 0 0 rgba(239,68,68,0.4)} 50%{box-shadow:0 0 0 8px rgba(239,68,68,0)}
        }
        @keyframes bot-reaction-pop {
        ...
        .bot-scroll { scrollbar-width:thin; scrollbar-color:rgba(109,120,250,0.2) transparent; overscroll-behavior:contain; }
        ...
        @media (min-width:601px) {
          .bot-msgs-inner { max-width:900px; margin:0 auto; width:100%; }
          .bot-input-inner { max-width:900px; margin:0 auto; width:100%; }
        }
        @media (max-width:600px) {
          ...
          .bot-hbtn-wrap > button { width:30px !important; height:30px !important; border-radius:8px !important; }
          .bot-send-btn { width:38px !important; height:38px !important; }
        }
      `}</style>

      {/* ── FAB ── */}
      {!open && (
        <div style={{ position:"fixed", bottom:`calc(env(safe-area-inset-bottom,0px) + 22px)`, right:"22px", zIndex:9998 }}>
          <div style={{
          ...
          {unreadCount > 0 && (
            <div style={{
              ...
              display:"flex", alignItems:"center", justifyContent:"center",
              fontFamily:MONO, fontSize:"8px", color:"white", fontWeight:700,
              padding:"0 3px",
            }}>{unreadCount}</div>
          )}
        </div>
      ...
      {/* ── PANEL — z-index 9999 above everything ── */}
      {open && (
        <div style={{
          ...
          zIndex:9999, // ← above everything including sidebar
        }}>
          ...
          <div className="bot-header-inner" style={{
            ...
            <div style={{ flex:1, minWidth:0 }}>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:"6px", marginTop:"2px", flexWrap:"wrap" }}>
                ...
                {!soundEnabled && <>
                  <span style={{ color:TEXT_DIM, fontSize:"8px" }}>·</span>
                  <span style={{ fontFamily:MONO, fontSize:"9px", color:TEXT_DIM }}>MUTE</span>
                </>}
                {loading && <>
                  <span style={{ color:TEXT_DIM, fontSize:"8px" }}>·</span>
                  <span style={{ fontFamily:MONO, fontSize:"9px", color:ACCENT3, animation:"bot-antenna-pulse 1s infinite" }}>THINKING</span>
                </>}
              </div>
            </div>
            ...
            <div className="bot-hbtns" style={{ display:"flex", gap:"4px", flexShrink:0, alignItems:"center" }}>
              ...
              {/* Sound toggle */}
              <div className="bot-hbtn-wrap">
                <HeaderBtn icon={soundEnabled ? <IconSound size={14}/> : <IconVolumeOff size={14}/>}
                  tooltip={soundEnabled ? "Sound on  (Ctrl+Shift+S)" : "Sound off  (Ctrl+Shift+S)"}
                  onClick={()=> setSoundEnabled(v=>!v)} variant="sound" active={soundEnabled}/>
              </div>
              <div className="bot-hbtn-wrap">
                <HeaderBtn icon={<IconVolume size={14}/>}
                  tooltip={selectedVoice ? `Voice: ${selectedVoice.name.split(" ").slice(0,2).join(" ")}` : "TTS settings"}
          ...
          {/* ── MESSAGES — with overscroll-behavior: contain to stop bg scroll ── */}
          <div ref={scrollContainerRef} className="bot-scroll" style={{
            flex:1, overflowY:"auto",
            padding:`16px ${chatPaddingH} 8px`,
            display:"flex", flexDirection:"column",
            position:"relative", zIndex:1,
            WebkitOverflowScrolling:"touch",
            overscrollBehavior:"contain",
          }}>
            <div className="bot-msgs-inner" style={{
              display:"flex", flexDirection:"column", gap: isMobile ? "16px" : "14px",
            }}>
              {messages.length===0 ? (
          ...
          {/* ── INPUT ── */}
          <div style={{
            padding:`12px ${chatPaddingH} calc(env(safe-area-inset-bottom,0px) + 14px)`,
            ...
            <div className="bot-input-inner">
              <div style={{
                ...
                {/* Voice input */}
                <button onClick={toggleVoice} title={listening?"Stop recording":"Voice input"} style={{
                ...
                <button className="bot-send bot-send-btn"
                  ...
                  style={{
                    ...
                    background: loading
                      ? "linear-gradient(135deg,rgba(239,68,68,0.3),rgba(239,68,68,0.18))"
                      : input.trim()
                        ? `linear-gradient(135deg,${ACCENT} 0%,${ACCENT3} 100%)`
                        : "rgba(255,255,255,0.04)",
                    border:`1px solid ${loading ? "rgba(239,68,68,0.4)" : input.trim() ? "rgba(109,120,250,0.55)" : BORDER}`,
                    color: (loading || input.trim()) ? "#fff" : TEXT_DIM,
                    cursor: (loading || input.trim()) ? "pointer" : "default",
                    display:"flex", alignItems:"center", justifyContent:"center",
                    fontSize:"15px",
                    boxShadow: loading ? "0 4px 18px rgba(239,68,68,0.3)" : input.trim() ? "0 4px 18px rgba(109,120,250,0.45)" : "none",
                    animation: (!loading && input.trim()) ? "bot-send-ready 2s ease-in-out infinite" : "none",
                    outline:"none", alignSelf:"flex-end",
                    transition:"all 0.18s cubic-bezier(0.22,1,0.36,1)",
                  }}
                  title={loading ? "Cancel response" : "Send message"}
                >
                  {loading ? <IconStop size={12}/> : "↑"}
                </button>
              </div>
              ...
              {!isMobile && (
                <div style={{
                  ...
                  {loading && (
                    <>
                      <span style={{ color:TEXT_DIM, opacity:0.2, fontSize:"8px" }}>·</span>
                      <span style={{ fontFamily:MONO, fontSize:"8px", color:"#fca5a5", opacity:0.7 }}>■ stop to cancel</span>
                    </>
                  )}
                </div>
              )}
          ...
          {showClearModal && (
            <ClearModal
              ...
              onConfirm={()=>{ setMessages([]); setShowClearModal(false); stopSpeak(); setBookmarks([]); setReactions({}); setStreamingIdx(null); abortRef.current?.abort(); setLoading(false); }}
